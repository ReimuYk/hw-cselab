...
fuseserver_readlink(fuse_req_t req, fuse_ino_t ino)
{
    ...
    std::string link;
    if(yfs->readlink(inum,link)!=yfs_client::OK){
        fuse_reply_err(req,ENOENT);
    }else{
...
fuseserver_symlink(fuse_req_t req, const char* link, fuse_ino_t parent, const char* name){
    yfs_client::inum inum;
    if (yfs->symlink(parent,name,link,inum)!=yfs_client::OK){
        fuse_reply_err(req,ENOENT);
    }else{
        fuse_entry_param ent;
        ent.attr_timeout = 0.0;
        ent.entry_timeout = 0.0;
        ent.generation = 0;
        ent.ino = inum;
        getattr(inum,ent.attr);
